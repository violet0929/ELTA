...
namespace ns3 {
    ...
    HtFrameExchangeManager::StartFrameExchange(Ptr<QosTxop> edca, Time availableTime, bool initialFrame) {
        ...
        // Check primary channel if AC_BE or AC_BK TXOP is obtained
        if (edca->GetAccessCategory() == AC_VI || edca->GetAccessCategory() == AC_BE){
            auto mpdu = GetBar(AC_VO);
            if(mpdu){
                NS_LOG_UNCOND("need retransmission, block TXOP" << m_self << (int)m_linkId);
                return false;
            }
        }
        m_mac->GetLinkIds();
    ...
    HtFrameExchangeManager::ForwardPsduDown(Ptr<const WifiPsdu> psdu, WifiTxVector &txVector) {
        ...
        for (int i = 0; i < (int) psdu->GetNMpdus(); i++) {
            ...
            if(mpdu_header.IsQosData()){
                wifiAnalyzer.SendQosData(Simulator::Now().As(Time::S), m_self, m_linkId, mpdu_header.GetAddr2(),
                ...
                wifiAnalyzer.SendNormalAck(Simulator::Now().As(Time::S), m_self, m_linkId, mpdu_header.GetAddr2(),
                                          mpdu_header.GetAddr1(),
                                          ptr[i]->GetSize(),
                                          ptr[i]->GetHeader().IsRetry());
            }
            else if(mpdu_header.IsBlockAck()){
                wifiAnalyzer.SendBlockAck(Simulator::Now().As(Time::S), m_self, m_linkId, mpdu_header.GetAddr2(),
                                          mpdu_header.GetAddr1(),
                                          ptr[i]->GetSize(),
                                          ptr[i]->GetHeader().IsRetry());
            }
            else if(mpdu_header.IsBlockAckReq()){
                wifiAnalyzer.SendBlockAckReq(Simulator::Now().As(Time::S), m_self, m_linkId, mpdu_header.GetAddr2(),
                                           mpdu_header.GetAddr1(),
                                           ptr[i]->GetSize(),
                                           ptr[i]->GetHeader().IsRetry());
            }
            else{
                wifiAnalyzer.SendOther(Simulator::Now().As(Time::S), m_self, m_linkId, mpdu_header.GetAddr2(),
                                             mpdu_header.GetAddr1(),
                                             ptr[i]->GetSize(),
                                             ptr[i]->GetHeader().IsRetry());
            }
