...
#include "elta-tracer.h"

namespace ns3
{

int ns3::EltaTracer::m_nSTA;

EltaTracer::EltaTracer()
{
}

EltaTracer::EltaTracer(int nSTA)
{
    m_nSTA = nSTA;
}
...
void
EltaTracer::SendQosData(TimeWithUnit unit,
                          Mac48Address device,
                          uint8_t linkId,
                          Mac48Address src,
                          Mac48Address dst,
                          ...
                          bool retry)
{
    ...
    if (((int)std::stoi(addr, nullptr, 16) - 1) / 3 + 1 == m_nSTA + 1)
    {
        subject = "AP";
        if (dst.IsBroadcast())
        {
        ...
        else
        {
            ...
            description = subject + " -> STA" +
                          toString(((int)std::stoi(dst_addr, nullptr, 16) - 1) / 3 + 1) +
                          ", QosDATA";
        }
    }
    else
...
void
EltaTracer::SendNormalAck(TimeWithUnit unit,
                            Mac48Address device,
                            uint8_t linkId,
                            Mac48Address src,
                            Mac48Address dst,
...
void
EltaTracer::SendBlockAck(TimeWithUnit unit,
                           Mac48Address device,
                           uint8_t linkId,
                           Mac48Address src,
                           Mac48Address dst,
...
void
EltaTracer::SendBlockAckReq(TimeWithUnit unit,
                              Mac48Address device,
                              uint8_t linkId,
                              Mac48Address src,
                              Mac48Address dst,
...
void
EltaTracer::SendOther(TimeWithUnit unit,
                        Mac48Address device,
                        uint8_t linkId,
                        Mac48Address src,
                        Mac48Address dst,
                        ...
                        bool retry)
{
    ...
    if (((int)std::stoi(addr, nullptr, 16) - 1) / 3 + 1 == m_nSTA + 1)
    {
        ...
        else
        {
            std::string dst_addr = toString(dst).substr(length - 2, 2);
            description = subject + " -> STA" +
                          toString(((int)std::stoi(dst_addr, nullptr, 16) - 1) / 3 + 1) + ", Other";
        }
    }
    else
    {
...
void
EltaTracer::ReceiveQosData(TimeWithUnit unit,
                             Mac48Address device,
                             uint8_t linkId,
                             Mac48Address src,
                             Mac48Address dst,
                             ...
                             bool retry)
{
    ...
    if (((int)std::stoi(addr, nullptr, 16) - 1) / 3 + 1 == m_nSTA + 1)
    {
        ...
        description = "STA" + toString(((int)std::stoi(src_addr, nullptr, 16) - 1) / 3 + 1) +
                      " <- " + subject + ", QosDATA";
    }
    else
    {
...
void
EltaTracer::ReceiveNormalAck(TimeWithUnit unit,
                               Mac48Address device,
                               uint8_t linkId,
                               Mac48Address src,
                               Mac48Address dst,
...
void
EltaTracer::ReceiveBlockAck(TimeWithUnit unit,
                              Mac48Address device,
                              uint8_t linkId,
                              Mac48Address src,
                              Mac48Address dst,
...
void
EltaTracer::ReceiveBlockAckReq(TimeWithUnit unit,
                                 Mac48Address device,
                                 uint8_t linkId,
                                 Mac48Address src,
                                 Mac48Address dst,
...
void
EltaTracer::ReceiveOther(TimeWithUnit unit,
                           Mac48Address device,
                           uint8_t linkId,
                           Mac48Address src,
                           Mac48Address dst,
...
void
EltaTracer::ConditionalIOStream(const std::string& subject,
                                  const int& linkId,
...
void
EltaTracer::IOStream(const std::string& subject,
                       const int& linkId,
                       const std::vector<std::string>& elements,
                       const std::string& delimiter)
{
