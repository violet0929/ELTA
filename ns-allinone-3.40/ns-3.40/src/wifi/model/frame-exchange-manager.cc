        ...
        if (psdu->GetNMpdus() == 1)
        {
            ...
            for (int i = 0; i < (int) psdu->GetNMpdus(); i++) {
                ...
                if(mpdu_header.IsQosData()){
                    eltaTracer.ReceiveQosData(Simulator::Now().As(Time::S), m_self, m_linkId, mpdu_header.GetAddr2(),
                    ...
                    eltaTracer.ReceiveNormalAck(Simulator::Now().As(Time::S), m_self, m_linkId, mpdu_header.GetAddr2(),
                                                  mpdu_header.GetAddr1(),
                                                  ptr[i]->GetSize(),
                                                  ptr[i]->GetHeader().IsRetry());
                }
                else if(mpdu_header.IsBlockAck()){
                    eltaTracer.ReceiveBlockAck(Simulator::Now().As(Time::S), m_self, m_linkId, mpdu_header.GetAddr2(),
                                                 mpdu_header.GetAddr1(),
                                                 ptr[i]->GetSize(),
                                                 ptr[i]->GetHeader().IsRetry());
                }
                else if(mpdu_header.IsBlockAckReq()){
                    eltaTracer.ReceiveBlockAckReq(Simulator::Now().As(Time::S), m_self, m_linkId, mpdu_header.GetAddr2(),
                                                    mpdu_header.GetAddr1(),
                                                    ptr[i]->GetSize(),
                                                    ptr[i]->GetHeader().IsRetry());
                }
                else{
                    eltaTracer.ReceiveOther(Simulator::Now().As(Time::S), m_self, m_linkId, mpdu_header.GetAddr2(),
                                              mpdu_header.GetAddr1(),
                                              ptr[i]->GetSize(),
                                              ptr[i]->GetHeader().IsRetry());
                }
