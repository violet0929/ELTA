            ...
            if (psdu->GetNMpdus() == 1) {
                ...
                for (int i = 0; i < (int) psdu->GetNMpdus(); i++) {
                    ...
                    if(mpdu_header.IsQosData()){
                        wifiAnalyzer.ReceiveQosData(Simulator::Now().As(Time::S), m_self, m_linkId, mpdu_header.GetAddr2(),
                        ...
                        wifiAnalyzer.ReceiveNormalAck(Simulator::Now().As(Time::S), m_self, m_linkId, mpdu_header.GetAddr2(),
                                                   mpdu_header.GetAddr1(),
                                                   ptr[i]->GetSize(),
                                                   ptr[i]->GetHeader().IsRetry());
                    }
                    else if(mpdu_header.IsBlockAck()){
                        wifiAnalyzer.ReceiveBlockAck(Simulator::Now().As(Time::S), m_self, m_linkId, mpdu_header.GetAddr2(),
                                                  mpdu_header.GetAddr1(),
                                                  ptr[i]->GetSize(),
                                                  ptr[i]->GetHeader().IsRetry());
                    }
                    else if(mpdu_header.IsBlockAckReq()){
                        wifiAnalyzer.ReceiveBlockAckReq(Simulator::Now().As(Time::S), m_self, m_linkId, mpdu_header.GetAddr2(),
                                                     mpdu_header.GetAddr1(),
                                                     ptr[i]->GetSize(),
                                                     ptr[i]->GetHeader().IsRetry());
                    }
                    else{
                        wifiAnalyzer.ReceiveOther(Simulator::Now().As(Time::S), m_self, m_linkId, mpdu_header.GetAddr2(),
                                               mpdu_header.GetAddr1(),
                                               ptr[i]->GetSize(),
                                               ptr[i]->GetHeader().IsRetry());
                    }
